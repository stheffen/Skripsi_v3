...
export default function InputNilaiClient({ user, filledSemesters }: { user: any, filledSemesters: number[] }) {
    ...
    <div className="flex flex-col sm:flex-row sm:items-center gap-4 px-4 py-4 hover:bg-slate-50 dark:hover:bg-slate-800/30 transition border-b border-slate-200 dark:border-slate-800/50 last:border-0">
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-1">
          ...
          <span className="text-xs px-1.5 py-0.5 bg-slate-100 text-slate-600 dark:bg-slate-700/50 dark:text-slate-400 rounded">{item.jenis}</span>
        </div>
      </div>
      <div className="self-end sm:self-auto mt-2 sm:mt-0">
        {item.nilai ? (
          <span className={`text-sm font-bold px-3 py-1.5 rounded-lg border shadow-sm dark:shadow-none ${NILAI_COLORS[item.nilai]}`}>
            {item.nilai}
          </span>
        ) : (
          <span className="text-xs text-slate-500 dark:text-slate-600 bg-slate-100 dark:bg-slate-800/50 px-2 py-1 rounded">Belum Ada Nilai</span>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Lihat Nilai</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm mt-1">Daftar mata kuliah dan nilai Anda per semester</p>
      </div>
        ...
        {filledSemesters.map(s => (
          <button
            ...
            className={`px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 flex-1 min-w-[70px] ${
              activeSem === s ? "bg-blue-600 text-white shadow-lg shadow-blue-500/20" : "text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-slate-200"
            }`}
          >
            Sem {s}
      ...
      {loading ? (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-16 flex items-center justify-center shadow-sm">
