...
const HasilRadarChart = dynamic(
  () => import("@/components/charts/HasilRadarChart"),
  {
    ssr: false,
    loading: () => (
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 h-[250px] flex items-center justify-center text-slate-500 text-sm animate-pulse">
        Memuat grafik...
      </div>
    ),
  },
...
function RiskBadge({ kategori, large }: any) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full font-bold tracking-wider uppercase ${cls} ${large ? "text-sm px-4 py-2" : "text-xs px-3 py-1"}`}
    >
...
function MembershipBar({ label, value, color }: any) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <span className="text-xs font-mono text-slate-600 dark:text-slate-300 w-10">
        {value.toFixed(3)}
      </span>
    </div>
  );
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-3 p-3 bg-slate-800/60 rounded-xl border border-slate-700">
        ...
        <div>
          <p className="text-sm font-semibold text-slate-200">
            {mkList.length} Mata Kuliah Belum Diambil
          </p>
          <p className="text-xs text-slate-400">
            Total:{" "}
            <span className="text-amber-400 font-bold">{totalSks} SKS</span>{" "}
            belum ditempuh
          </p>
        </div>
      </div>
                ...
                <td colSpan={5}>
                  <div className="rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
                    <div className="bg-slate-50 dark:bg-slate-800/80 px-3 py-2 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
                      ...
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium bg-slate-200 dark:bg-slate-900 px-2 py-0.5 rounded-md">
                        {mkSem.length} MK
                      </span>
                    </div>
                    <table className="w-full text-xs">
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50 bg-white dark:bg-transparent">
                        {mkSem.map((mk: any, i: number) => (
                          ...
                          >
                            <td className="px-3 py-2 font-mono text-slate-500 dark:text-slate-400 w-24">
                              {mk.kode}
                            </td>
                            <td className="px-3 py-2 text-slate-800 dark:text-slate-300 font-medium">
                              {mk.nama}
                            </td>
                            <td className="px-3 py-2 text-center text-slate-500 dark:text-slate-400 w-12">
                              {mk.sks} SKS
                            </td>
                            <td className="px-3 py-2 text-right w-24">
                              <span
                                className={`px-2 py-0.5 rounded-md text-[10px] font-bold tracking-wide uppercase ${mk.jenis === "Wajib" ? "bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300" : "bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-300"}`}
                              >
                                {mk.jenis}
                              </span>
                            </td>
      ...
      {mkList.length > 10 && (
        <button
          ...
          className="w-full py-2 text-xs text-blue-400 hover:text-blue-300 transition flex items-center justify-center gap-1"
        >
          {showAll ? (
            <>
            ...
            <>
              <ChevronDown size={14} /> Tampilkan semua {mkList.length} mata
              kuliah
            </>
          )}
        </button>
...
export default function HasilAnalisisClient({
  user,
  initialData,
  maxSemester = 1,
  mkBelumDiambil = [],
  ...
  const [semester, setSemester] = useState(
    Math.min(user?.semester_aktif || 1, maxSemester),
  );
  ...
  const handleAnalisis = async () => {
    ...
    try {
      ...
      if (res.error || !res.data)
        throw new Error(res.error || "Data tidak ditemukan");
  ...
  const fuzzyDetail = result?.detail_fuzzy
    ? (() => {
        try {
          return JSON.parse(result.detail_fuzzy);
        } catch {
          return null;
        }
      })()
  ...
  const radarData = result
    ? [
        ...
        { subject: "IPK", A: (result.ipk / 4) * 100, fullMark: 100 },
        {
          subject: "MK Aman",
          A:
            result.mk_bermasalah === 0
              ? 100
              : Math.max(0, 100 - result.mk_bermasalah * 15),
          fullMark: 100,
        },
      ]
    : [];
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">
          Analisis Risiko Akademik
        </h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm mt-1">
          Perhitungan menggunakan metode Logika Fuzzy Mamdani
        </p>
      </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
          <div>
            <p className="text-sm font-semibold text-slate-900 dark:text-slate-200 mb-1">
              Hitung Analisis Baru
            </p>
            <p className="text-xs text-slate-500">
              Pilih semester (maks. Semester {maxSemester} berdasarkan angkatan{" "}
              {user?.angkatan || "-"})
            </p>
          </div>
      ...
      {!result ? (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-16 text-center">
          ...
          <p className="text-slate-900 dark:text-slate-300 font-medium mb-2">
            Belum ada analisis risiko
          </p>
          <p className="text-slate-500 text-sm">
            Pilih semester dan klik "Hitung Analisis" untuk memulai
          </p>
        </div>
      ) : (
        <>
          ...
          <div
            className={`rounded-2xl p-6 border shadow-sm ${
              result.kategori === "Tinggi"
                ? "bg-white dark:bg-slate-900 border-l-4 border-l-risk-tinggi dark:border-risk-tinggi/30 dark:shadow-[0_0_20px_var(--color-risk-tinggi)]"
                : result.kategori === "Sedang"
                  ? "bg-white dark:bg-slate-900 border-l-4 border-l-risk-sedang dark:border-risk-sedang/30 dark:shadow-[0_0_20px_var(--color-risk-sedang)]"
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
              <div>
                <p className="text-xs text-slate-500 dark:text-slate-400 mb-2">
                  Hasil Analisis Semester {result.semester}
                </p>
                ...
                {angkatanMode && (
                  <span
                    className={`mt-2 inline-block text-xs px-2 py-0.5 rounded-full border ${angkatanMode === "baru_9_aturan" ? "bg-purple-500/20 text-purple-300 border-purple-500/30" : "bg-slate-700 text-slate-400 border-slate-600"}`}
                  >
                    {angkatanMode === "baru_9_aturan"
                      ? "? 9 Aturan (Angkatan 2026+)"
                      : "?? 27 Aturan (Angkatan <2026)"}
                  </span>
                )}
              </div>
              <div className="sm:ml-auto grid grid-cols-3 gap-6 text-center w-full sm:w-auto mt-4 sm:mt-0">
                <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-100 dark:border-slate-800/50">
                  <p className="text-2xl font-bold text-slate-900 dark:text-slate-100">
                    {result.ips?.toFixed(2) ?? "-"}
                  </p>
                  <p className="text-xs text-slate-500 mt-1">IPS</p>
                </div>
                <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-100 dark:border-slate-800/50">
                  <p className="text-2xl font-bold text-slate-900 dark:text-slate-100">
                    {result.ipk?.toFixed(2) ?? "-"}
                  </p>
                  <p className="text-xs text-slate-500 mt-1">IPK</p>
                </div>
                <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-100 dark:border-slate-800/50">
                  <p
                    className={`text-2xl font-bold ${(result.mk_bermasalah_detail?.length ?? result.mk_bermasalah) > 0 ? "text-red-600 dark:text-red-400" : "text-emerald-600 dark:text-emerald-400"}`}
                  >
                    {result.mk_bermasalah_detail?.length ??
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-6">
                  <div>
                    <p className="text-xs text-slate-400 mb-2 font-medium">
                      IPS = {result.ips?.toFixed(2)}
                    </p>
                    <div className="space-y-1.5">
                      <MembershipBar
                        label="Rendah"
                        value={fuzzyDetail.fuzzifikasi.ips.rendah}
                        color="#ef4444"
                      />
                      <MembershipBar
                        label="Sedang"
                        value={fuzzyDetail.fuzzifikasi.ips.sedang}
                        color="#f59e0b"
                      />
                      <MembershipBar
                        label="Tinggi"
                        value={fuzzyDetail.fuzzifikasi.ips.tinggi}
                        color="#10b981"
                      />
                    </div>
                  </div>
                  <div>
                    <p className="text-xs text-slate-400 mb-2 font-medium">
                      IPK = {result.ipk?.toFixed(2)}
                    </p>
                    <div className="space-y-1.5">
                      <MembershipBar
                        label="Rendah"
                        value={fuzzyDetail.fuzzifikasi.ipk.rendah}
                        color="#ef4444"
                      />
                      <MembershipBar
                        label="Sedang"
                        value={fuzzyDetail.fuzzifikasi.ipk.sedang}
                        color="#f59e0b"
                      />
                      <MembershipBar
                        label="Tinggi"
                        value={fuzzyDetail.fuzzifikasi.ipk.tinggi}
                        color="#10b981"
                      />
                    </div>
                  </div>
                  {angkatanMode !== "baru_9_aturan" &&
                    fuzzyDetail.fuzzifikasi.mk && (
                      <div>
                        <p className="text-xs text-slate-400 mb-2 font-medium">
                          Input Fuzzy MK Bermasalah ={" "}
                          {result.mk_bermasalah_detail?.length ??
                            result.mk_bermasalah}
                        </p>
                        <div className="space-y-1.5">
                          <MembershipBar
                            label="Sedikit"
                            value={fuzzyDetail.fuzzifikasi.mk.sedikit}
                            color="#10b981"
                          />
                          <MembershipBar
                            label="Sedang"
                            value={fuzzyDetail.fuzzifikasi.mk.sedang}
                            color="#f59e0b"
                          />
                          <MembershipBar
                            label="Banyak"
                            value={fuzzyDetail.fuzzifikasi.mk.banyak}
                            color="#ef4444"
                          />
                        </div>
                      </div>
            ...
            <HasilRadarChart
              ...
              fuzzy_output={result.fuzzy_output}
            />
          </div>
          ...
          {result.mk_bermasalah_detail?.length > 0 && (
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 hover:border-slate-300 dark:hover:border-slate-700 transition-colors shadow-sm">
              <h3 className="font-semibold text-slate-900 dark:text-slate-200 mb-4 text-sm flex items-center gap-2">
                ...
                Mata Kuliah Perlu Diperbaiki (
                {result.mk_bermasalah_detail.length} MK)
              </h3>
